...
import { downloadBlob, generateMarcelloMaternityContract } from "@/lib/contracts-client";
...
const EMPTY_FORM: MaternityMarcelloContractData = {
  fullName: "",
  cpf: "",
  maritalStatus: "",
  profession: "",
...
export function MaternityMarcelloForm() {
  const router = useRouter();
  const { accessToken } = useAuth();

  const [form, setForm] = useState<MaternityMarcelloContractData>(EMPTY_FORM);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const payload: MaternityMarcelloContractData = {
        ...form,
        rg: form.rg?.trim() || undefined,
        rgState: form.rgState?.trim() || undefined,
        streetNumber: form.streetNumber?.trim() || undefined,
        email: form.email?.trim() || undefined,
        phone: form.phone?.trim() || undefined,
        guardianName: form.isMinor ? form.guardianName?.trim() || undefined : undefined,
        guardianCpf: form.isMinor ? form.guardianCpf?.trim() || undefined : undefined,
      };
      const blob = await generateMarcelloMaternityContract(accessToken, payload);
      const filename = `CONTRATO MARCELLO - ${form.fullName.toUpperCase()}.pdf`;
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="border-b border-border/40 px-6 py-5">
        <div className="mb-3 flex items-center gap-2 text-xs text-muted-foreground/60">
          ...
          <span>/</span>
          <span className="text-foreground/80">Salário Maternidade — Marcello Renault</span>
        </div>
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-semibold text-foreground">
            Contrato Salário Maternidade — Marcello Renault
          </h1>
        </div>
        <p className="mt-0.5 text-xs text-muted-foreground/60">
          Campos marcados como <span className="text-muted-foreground/40">opcional</span> serão omitidos do contrato se não preenchidos.
        </p>
      </div>
